...
class Trapezoid : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
        ...
        float tmpOmega = lastOmega - g * Mathf.Sin(lastTheta) / length * Time.deltaTime * step;
        float tmpTheta = lastTheta +  lastOmega * Time.deltaTime * step;

        float midOmega = (lastOmega + tmpOmega) / 2;
        float midTheta = (lastTheta + tmpTheta) / 2;
        theta = theta + midOmega * Time.deltaTime * step;

        omega = omega - g * Mathf.Sin(midTheta) * Time.deltaTime * step / length;
        deltaTheta = theta - lastTheta;
        //move the object to the new postion
