...
class midpoint : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
        ...
        float tmpOmega = lastOmega - 0.5f * g * Mathf.Sin(lastTheta) / length * Time.deltaTime * step;
        float tmpTheta = lastTheta + 0.5f * lastOmega * Time.deltaTime * step;

        theta = theta + tmpOmega * Time.deltaTime * step;

        omega = omega - g * Mathf.Sin(tmpTheta) * Time.deltaTime * step / length;//使用tmpTheta更新omega
